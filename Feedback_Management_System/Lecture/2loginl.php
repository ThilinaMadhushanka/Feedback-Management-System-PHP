...
<body>
    <div class="wrapper">
        ...
        <form action="2registerl.php" class="form-register">
            <div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $query = "SELECT * FROM lecture WHERE username = '$username' AND password = '$password'";
        $result = mysqli_query($con, $query);

        if (mysqli_num_rows($result) == 1) {
            header("Location: 2lectureHome.php");
            exit();
        } else {
            header("Location: 2loginl.php?error=invalid_credentials");
            exit();
        }
    }
    ?>
